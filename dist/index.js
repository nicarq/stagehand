...
2. ${isUsingAccessibilityTree ? "a hierarchical accessibility tree showing the semantic structure of the page" : "a numbered list of possible elements or an annotated image of the page"}
...
function getBrowser(apiKey, projectId, env = "LOCAL", headless = false, logger, browserbaseSessionCreateParams, browserbaseSessionID, executablePath, downloadPath) {
  return __async(this, null, function* () {
    if (env === "BROWSERBASE") {
      if (!apiKey) {
        logger({
    ...
    if (env === "BROWSERBASE") {
      ...
      const downloadsPath = downloadPath || import_path2.default.join(process.cwd(), "downloads");
